...
class QueryMode(str, Enum):
    GLOBAL = "global"   # search Neon (permanent store)
    LOCAL  = "local"    # search Redis (session store)
    BOTH   = "both"     
...
def read_image_to_base64(file_bytes: bytes, content_type: str) -> str:
    ...
    if content_type not in ALLOWED_IMAGE_TYPES:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported image type: {content_type}. Allowed: {ALLOWED_IMAGE_TYPES}"
        )
...
def decode_base64_to_image(data_url: str):
    ...
    if not data_url.startswith("data:") or ";base64," not in data_url:
        raise HTTPException(
            status_code=500,
            detail="Stored image format is invalid or corrupted"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Could not decode stored image: {e}"
        )
...
def store_chunks_local_redis(
    ...
    if not redis_client:
        raise HTTPException(
            status_code=500,
            detail="Local session store (Redis) unavailable."
        )
...
async def get_upload_consent(
    ...
    if upload_mode == UploadMode.GLOBAL:
        ...
        return ConsentMessage(
            ...
            message=(
                "You have selected Global Upload. "
                "Your document will be permanently stored and made available "
                "to all authorised users within your organisation. "
                "It will remain accessible until manually removed by an Admin. "
                "Please ensure the document complies with your organisation's "
    ...
    return ConsentMessage(
        ...
        message=(
            "You have selected Local Upload. "
            "Your document will be stored privately for this session only — "
            "no other user, including Admins and Super Admins, can access or query it. "
            "After 1 hour (session expiry), your data will be automatically "
            "and permanently erased. It cannot be recovered after that point."
        ),
...
async def upload_document(
    ...
    if not confirmed:
        raise HTTPException(
            ...
            detail=(
                ...
                "Please confirm the upload mode disclaimer first."
            )
        )
    ...
    if upload_mode == UploadMode.GLOBAL:
        if current_user.role not in [Role.ADMIN, Role.SUPER_ADMIN]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Global uploads require Admin or Super Admin role."
            )
    ...
    if not org_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="User has no organisation assigned. Contact your Admin."
        )
    ...
    try:
        raw_text, total_pages = extract_text_by_type(
            doc_type, file_bytes, filename
        )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=400,
            detail=f"Content extraction failed for '{filename}': {e}"
        )
    ...
    if upload_mode == UploadMode.GLOBAL:
        ...
        return UploadReport(
            ...
            org_id=org_id,
            doc_type=doc_type.value,
            total_pages=delta["total_pages"],
            pages_newly_indexed=delta["pages_newly_indexed"],
            pages_skipped=delta["pages_skipped"],
            chunks_created=delta["chunks_created"],
            status="indexed_globally" if delta["is_new_document"] else "delta_indexed",
        ...
        return UploadReport(
            ...
            doc_type=doc_type.value,
            total_pages=total_pages,
            pages_newly_indexed=total_pages, 
            pages_skipped=0,                 
            chunks_created=chunks_created,
            status="indexed_locally_session_ttl_1hr",
            uploaded_at=uploaded_at
        )                                                                                                                                                                                                                           
...
async def query_rag(
    ...
    log_query_event(
        ...
        sources_found=len(retrieved_chunks),
        ip_address=ip_address
    )
    ...
    sources = [
        SourceResult(
            ...
            chunk_index=c["chunk_index"], similarity_score=round(c["similarity_score"], 4),
    ...
    return RAGResponse(
        ...
        query=body.query,
        language=body.language,
        query_mode=body.upload_mode,
        sources=sources,
        total_sources_found=len(sources),
        generated_by="llama-3.3-70b-versatile (Groq)",
        org_id=org_id,
